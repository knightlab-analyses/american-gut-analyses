...
gunzip raw-sequences.fna.gz

deblur workflow --seqs-fp raw-sequences.fna --output-dir ag-deblurred-100nt --jobs-to-start 8 --trim-length 100 &
deblur workflow --seqs-fp raw-sequences.fna --output-dir ag-deblurred-125nt --jobs-to-start 8 --trim-length 125 &
deblur workflow --seqs-fp raw-sequences.fna --output-dir ag-deblurred-150nt --jobs-to-start 8 --trim-length 150 &
...
curl -o gg-13-8-99-515-806-nb-classifier.qza https://data.qiime2.org/2017.4/common/gg-13-8-99-515-806-nb-classifier.qza
curl -o expand.py https://raw.githubusercontent.com/wasade/reimagined-fiesta/master/expand.py

curl -o sepp-package.tar.bz https://raw.github.com/smirarab/sepp-refs/master/gg/sepp-package.tar.bz
...
do
    ...
    source activate qiime191
    python -c "import biom; t = biom.load_table('$f'); f = open('$output', 'w'); f.write([''.join('>%s\n%s\n' % (i, i.upper())) for i in t.ids(axis='observation')])"
	cp $fna $cwd/
		
	run-sepp.sh $fna $base
	cp ${base}*.tog.tre ${base}*.json $cwd/
	
    pick_otus.py -i $fna -o ${base}_gg_cr99 -m sortmerna -s 0.99
	...
	table_expanded=$(basename ${table} .biom)_gg99.biom
